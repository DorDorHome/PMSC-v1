...
if __name__ == "__main__":
    ...
    if args.train_noise:
        ...
        noise_trainable=noise_original
    ...
    if not os.path.exists('projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}'.format(args.arch, args.ckpt)):
        os.makedirs('projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}'.format(args.arch, args.ckpt))
    
    utils.save_image(img_before_training, 'projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}/initial_before_training_targeting{}_seed{}.png'.format(args.arch,args.ckpt, target_real_img_index ,args.random_seed),
                                 normalize=True,
            range=(-1, 1),
            )
    ...
    if args.train_noise:
        optimizer = torch.optim.Adam([z_trainable] + noise_trainable, lr = learning_rate)
    else:
        optimizer = torch.optim.Adam([z_trainable], lr = learning_rate)

    for epoch in pbar:
            
        if args.arch =='hypernetwork_with_2_layer_FC_offset_middle_dim_512_32' or args.arch =='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32':
            img_generated, _ = g_ema(z_trainable, truncation=args.truncation, truncation_correlated_latent=mean_latent,
                noise = noise_trainable
                )
        else:
        ...
        loss = F.mse_loss(img_generated, img_target)
        ...
        if epoch % 1000 ==0:
            ...
            utils.save_image(latest_img, 'projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}/results_after_{}_epoch_targeting{}_seed{}.png'.format(args.arch,args.ckpt, epoch,target_real_img_index ,args.random_seed),
                            normalize=True,
                            range=(-1, 1),
            )  
        ...
        #     utils.save_image(img_before_training, 'projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}/reality_check_after_{}_epoch_targeting{}_seed{}.png'.format(args.arch , args.ckpt, epoch,target_real_img_index ,args.random_seed),
        #         normalize=True,
        #         range=(-1, 1),
        #     )  
        ...
        if epoch % 10000 ==0 or epoch == epochs-1:
            torch.save(z_trainable,'projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}/z_latent_after_{}_epoch_targeting{}_seed{}.npy'.format(args.arch, args.ckpt, epoch,target_real_img_index ,args.random_seed ))
            if args.train_noise:
                torch.save(noise_trainable,'projection_to_latent_space_results/find_latent_for_real_images_results/{}_with_ckpt_{}/trained_noise_after_{}_epoch_targeting{}_seed{}.npy'.format(args.arch, args.ckpt, epoch,target_real_img_index ,args.random_seed ))
