...
class TwoLayerHypernetwork(nn.Module):
    ...
    def forward(self, prev_correlated_w, current_w):
        ...
        #print('target out_dim :', self.out_dim)
        hyper_out1 = hyper_out1.view(current_w.shape[0], self.in_dim_current + 1, self.out_dim) # shape (batch, in_dim_current + 1, out_dim )
        #print('shape of hyper_out1 after reshaped ', hyper_out1.shape)
        #print('slicing shape: ', hyper_out1[:, :self.in_dim_current, :].shape)
        current_w = current_w.view(current_w.shape[0], 1,-1) # shape (batch, 1, in_dim_current)
        # print('shape of current_w after reshaping, before matmul:', current_w.shape)
        # print('shape of hyper_out mat part: ', hyper_out1[:, :self.in_dim_current, :].shape)
        # print('shape of output of mat mul: ', torch.matmul(current_w, hyper_out1[:, :self.in_dim_current, :]).shape)
        # print('shape of the bias output', hyper_out1[:, self.in_dim_current, :].shape)
        out = torch.matmul(current_w, hyper_out1[:, :self.in_dim_current, :])       + hyper_out1[:, self.in_dim_current: self.in_dim_current + 1, :]     # shape ( batch, 1, out_dim)
        #print('initial shape of output from hypernetwork', out.shape)
        out = out.view(current_w.shape[0], -1)
        #print('shape of ouput from hypernetwork ', out.shape)
...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        for i in range( self.num_layers):
            self.FC_from_w_to_correction_for_next.append(
                TwoLayerHypernetwork(in_dim_prev= w_style_dim, in_dim_current=w_style_dim, out_dim= w_style_dim, middle_dim=  n_neurons_middle_layer, bias_init=0, hidden_layer_activation = mapping_network_hidden_layer_activation, final_activation = mapping_network_final_layer_activation)
            )
