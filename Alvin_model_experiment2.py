# same as Alvin_model_experiment, except the following:
# 1. demodulate is also set to true in ToRGB.
# 2. self.scale is used in ToRGB. 
# the justification is that style alone is enough to change the 'norm' of the filter
# and scale is need to adjust for indiscrepancy in input  
...
class StyleEqualLinear(nn.Module):
    ...
    The difference is that the output of StyleEqualLinear is a softmax output
    ...
    def __init__(
        ...
        # defining the parameters on how much the style vector should be magnified before going through softmax;
        if bias:
            # one factor for each out_dim
            self.bias = nn.Parameter(torch.ones(out_dim).fill_(bias_init))

        else:
            self.bias = None

        # initialize the activation function. Default is softmax
    ...
    def forward(self, input):
        ...
        if self.activation == 'softmax': 
            ...
            out = F.linear(input, self.weight * self.scale) # shape: (batch dim, out_dim )
...
class NewModulatedConv2d(nn.Module):
    def __init__(
        ...
        self.activation = activation_for_style
        ...
        self.modulation = StyleEqualLinear(style_dim, in_channel, style_mul_factor=1, bias_init=1, activation= activation_for_style
    )
    ...
    def forward(self, input, style):
        ...
        if self.demodulate:
            ...
            weight = self.weight #*self.style # when demodulate not used, do we need to adjust for the number of in channels 
        ...
        if self.activation == 'softmax':
            ...
            weight = weight*style*self.scale
