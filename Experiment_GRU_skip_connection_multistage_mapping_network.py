...
class ConstantInput(nn.Module):
    ...
    def forward(self, input):
        #print('shape of input to the constant layer:', input.shape)
        batch = input.shape[0]
...
class MyNewGenerator(nn.Module):
    ...
    def forward(
       ...
       #print('shape of input latent:', latent.shape)
        ...
        #print('shape of input latent:', latent.shape)
        zero_style = torch.zeros([latent.shape[0], self.channels[4]]).to('cuda:1') # need to edit! 
        # print('shape of zero_style:', zero_style.shape )
        ...
        _ , out, transformed_style_0 = self.conv1(zero_style, out, inter_transformed_latent[0], noise=noise[0])
        h1 = torch.zeros([latent.shape[0], self.prev_style_hidden_dim]).to('cuda:1')
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            h_skip_1 = (h_skip + h_skip_1)/math.sqrt(2) # save current hidden state, as input to the next to_rgb 
            i += 2
