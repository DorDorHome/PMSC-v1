...
class GRU_Gaussian_map_with_prev_style(nn.Module):
    """
    ...
    def __init__(self, prev_style_hidden_dim, current_w_in_dim, current_out_style_dim, bias= True, final_activation = None, intermediate_activation = None ):
        super().__init__()
    # the function that map s1 to a mean:
        ...
        self.GRU = nn.GRUCell(input_size = current_w_in_dim, hidden_size = prev_style_hidden_dim, bias = bias ) # used to track previous states.
        # this is used to find the mean (bias) of the current output, purely based on 
        # previous hidden state 
        self.find_mean = EqualLinear(in_dim = prev_style_hidden_dim, out_dim = current_out_style_dim, activation = intermediate_activation) 

        self.find_var_map = EqualLinear(in_dim = prev_style_hidden_dim, out_dim = current_out_style_dim*current_w_in_dim, activation = intermediate_activation )    # this represents the final stage of the map, using h2 to calculates s2
    ...
    def forward(self, h1, w2):
        ...
        """
        # first we find the updated hidden state:
        h2 = self.GRU(w2, h1)
        # print('check input inside GRU Gauss:', type(h1))
        #next we compute the mean, based purely on the previous hidden state:
        bias = self.find_mean(h1)
        var_map = self.find_var_map(h1).view(h1.shape[0], self.current_w_in_dim, self.current_out_style_dim)

        # reshape w2:
        w2 = w2.view(w2.shape[0], 1, -1)

        out = torch.matmul(w2, var_map).view(w2.shape[0], -1) + bias
        if self.activation =='sqrt_softmax':
            out = F.softmax(out, dim =1)
            s2 =torch.sqrt(out)
        if self.activation == 'spherical':
            norm_factor = torch.rsqrt(out.pow(2).sum([1]))
            s2 = out*norm_factor.view(out.shape[0],1)
        else:
            s2 = out
...
class GRUModulatedConv2d(nn.Module):
    def __init__(
        ...
        if use_external_affine:
            ...
            self.map_style_to_hidden = EqualLinear(in_dim = w_style_dim, out_dim = prev_style_hidden_dim,activation = 'tanh')
        else:
            self.modulation = GRU_Gaussian_map_with_prev_style(prev_style_hidden_dim= prev_style_hidden_dim,
                                                            current_w_in_dim = w_style_dim,current_out_style_dim = in_channel,
                                                            final_activation= activation_for_style, intermediate_activation = intermediate_activation)
    ...
    def forward(self,h1 , input, current_w_style):
        ...
        if not self.fused:  # not the default
            ...
            if self.use_external_affine:
                ...
                h2 = self.map_style_to_hidden(style)
                if self.activation_for_style=="sqrt_softmax":
                    style = F.softmax(style, dim = 2)
                    style = torch.sqrt(style)
            else:
                h2, style = self.modulation(h1,current_w_style) # output shape: (batch, in_channel)
            ...
            return h2, out, styles.view(batch, self.in_channel)
        ...
        if self.use_external_affine:
            h2 = self.map_style_to_hidden(current_w_style)
...
class ToRGB(nn.Module):
    def __init__(self, in_channel, prev_style_hidden_dim , w_style_dim, upsample=True, blur_kernel=[1, 3, 3, 1], activation_for_style = None, intermediate_activation = None):
        super().__init__()

        if upsample:
            self.upsample = Upsample(blur_kernel)

        self.conv = GRUModulatedConv2d(in_channel, 3, 1, prev_style_hidden_dim, w_style_dim,demodulate=False, activation_for_style = activation_for_style, intermediate_activation = intermediate_activation)# , activation_for_style= None)
...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        final_activation = None) # none, because activation is included in self.conv1
    ...
    def forward(
        ...
        h1, out, transformed_style_0 = self.conv1(zero_style, out, inter_transformed_latent[0], noise=noise[0])
        ...
        # print('output type of first conv layer', type(out))
        # print('first hidden state output ', type(h1))
        h_skip_1, skip, transformed_style_skip1 = self.to_rgb1(h1, out, inter_transformed_latent[1])
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            h2, out, transformed_style2 = conv1(h1, out, inter_transformed_latent[i], noise=noise1)
            # print(f'\n computing {i}-th conv layer: \n')
            h3, out, transformed_style3 = conv2(h2, out, inter_transformed_latent[i + 1], noise=noise2)
            # print(f'\n computing {i}-th rgb layer \n ')
            h_skip, skip, transformed_style_skip = to_rgb(h_skip_1, out, inter_transformed_latent[i + 2], skip)
