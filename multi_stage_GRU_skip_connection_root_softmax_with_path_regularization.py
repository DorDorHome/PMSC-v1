...
class GRUModulatedConv2d(nn.Module):
    def __init__(
        ...
        if use_external_affine:
            ...
            self.affine_for_h = EqualLinear(w_style_dim, prev_style_hidden_dim, bias_init=1)
        else:
    ...
    def forward(self,h1 , input, current_w_style):
        ...
        if not self.fused:  # not the default
            ...
            if self.use_external_affine:
                ...
                h2 = self.affine_for_h(current_w_style)
            else:
        ...
        #sto (batch, in_channel) via the FC layer (self.modulation)
