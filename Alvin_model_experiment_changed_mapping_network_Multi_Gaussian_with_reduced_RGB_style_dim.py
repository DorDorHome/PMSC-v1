...
class Gaussian_affine_map_with_prev_style(nn.Module):
    ...
    def __init__(self, prev_transformed_style_dim, current_w_in_dim, current_out_style_dim ):
        super().__init__()
    # the function that map s1 to a mean:
        ...
        self.bias_map = EqualLinear(in_dim = prev_transformed_style_dim, out_dim = current_out_style_dim, activation = None)

        # the function that map s1 to a matrix of shape (current_out_style_dim, current_in_dim ) (but flattened)
        self.transform_matrix = EqualLinear(in_dim = prev_transformed_style_dim, out_dim = current_out_style_dim*current_w_in_dim, activation = None) 
...
class GaussianModulatedConv2d(nn.Module):
    def __init__(
        ...
        if use_external_affine:
            ...
            self.modulation = Gaussian_affine_map_with_prev_style(prev_transformed_style_dim = prev_transformed_style_dim, current_w_in_dim= w_style_dim, current_out_style_dim = in_channel)
...
class ToRGB(nn.Module):
    def __init__(self, in_channel, prev_transformed_style_dim , w_style_dim, upsample=True, blur_kernel=[1, 3, 3, 1]):
        super().__init__()

        if upsample:
            self.upsample = Upsample(blur_kernel)

        self.conv = GaussianModulatedConv2d(in_channel, 3, 1, prev_transformed_style_dim, w_style_dim,demodulate=False)# , activation_for_style= None)
...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_rgbs.append(ToRGB(out_channel, out_channel, w_style_dim))
