...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        self.to_rgb1 = ToRGB(self.channels[4], 2*w_style_dim, upsample=False)
        ...
        self.FC_from_w_to_correction_for_next.append(
                TwoLayerFC(in_dim= w_style_dim, out_dim= w_style_dim, middle_dim=  2*n_neurons_middle_layer, bias_init=0, activation = "fused_lrelu")
            )

        for i in range( self.num_layers):
            self.FC_from_w_to_correction_for_next.append(
                TwoLayerFC(in_dim= w_style_dim, out_dim= w_style_dim, middle_dim=  n_neurons_middle_layer, bias_init=0, activation = "fused_lrelu")
            )
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(
                    out_channel, out_channel, 3, 2*w_style_dim, blur_kernel=blur_kernel
                )
            )

            self.to_rgbs.append(ToRGB(out_channel, 2*w_style_dim))
    ...
    def mean_correlated_latent(self, n_latent):
        ...
        #     n_latent, self.w_style_dim, device=self.input.input.device
        # )
    ...
    def get_correlated_latent(self, latent): # originally, 'input' is used in place of 'latent'
        ...
        input:
        latent (w): a single tensor of shape (batch, self.w_style_dim*self.num_layers)
        ...
        for i in range(self.num_layers+1):
            latent_list.append(latent[:, i*self.w_style_dim:(i*self.w_style_dim+self.w_style_dim)])
