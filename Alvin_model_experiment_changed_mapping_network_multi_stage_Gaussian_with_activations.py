...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation=="fused_lrelu":
            ...
            # debug:
            # print('debug inside EqualLinear')
            # print('shape of input in EqualLinear:', input.shape)
            # print('shape of the weight in EqualLinear: ', self.weight.shape)
            out = F.linear(
                input, self.weight * self.scale, bias=self.bias * self.lr_mul
...
class ThreeLayerFC(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, hidden_dim_1st, hidden_dim_2nd, bias_init = 0, lr_mul = 1, activation = None):
        super().__init__()
        self.FC1 = EqualLinear(in_dim=in_dim, out_dim=hidden_dim_1st, bias = True, bias_init = bias_init, lr_mul = lr_mul, activation= activation)
        self.FC2 = EqualLinear(in_dim= hidden_dim_1st, out_dim= hidden_dim_2nd, bias = True, bias_init = bias_init, lr_mul = lr_mul, activation= activation)
        self.FC3 = EqualLinear(in_dim=hidden_dim_2nd, out_dim= out_dim, bias = True, bias_init = bias_init, lr_mul = lr_mul, activation= activation)
...
class Gaussian_affine_map_with_prev_style(nn.Module):
    """
    
    Created by Alvin. This is a network that, given a style vector from the previous layer( s1), 
    It creates:
    1. a mean vector based on s1
    2. a matrix that depends on s1, but serves as matrix that multiply on w2 in the forward pass.

    when w2 is sampled from Gaussian N(0, I), this gives a vector sample from:
    s2 ~ N(\mu(s1), \Lambda(s1)^T \Lambda(s1))
    """

    def __init__(self, prev_transformed_style_dim, current_w_in_dim, current_out_style_dim, activation = None ):
        super().__init__()
    # the function that map s1 to a mean:
        self.prev_transformed_style_dim = prev_transformed_style_dim
        self.current_out_style_dim = current_out_style_dim
        self.current_w_in_dim = current_w_in_dim
        self.activation = activation
        self.bias_map = EqualLinear(in_dim = prev_transformed_style_dim, out_dim = current_out_style_dim, activation = activation)
    ...
    def forward(self, s1, w2):
        # check that batch size agrees:
        #assert s1.shape[0] == w2.shape[0], "batch size of the prev style and current style doesn't agree!"
        # print('shape of s1:', s1.shape)
        # print('self.prev_transformed_style_dim', self.prev_transformed_style_dim)
        # print('self.current_out_style_dim:', self.current_out_style_dim)
        # print('self.current_w_in_dim', self.current_w_in_dim)

        bias = self.bias_map(s1)

        # print('finished bias map inside Gaussian affine\n')
        # print('starting transform matrix inside Gaussian affine\n')
        transform_matrix = self.transform_matrix(s1).view(s1.shape[0], self.current_w_in_dim, self.current_out_style_dim) # change to shape (current_out_style_dim, current_in_dim)
        # print('shape of transform_matrix after reshaped: ', transform_matrix.shape)
        # print('finished transform matrix inside gaussian affine\n')
        # print('initial shape of transform matrix: ', transform_matrix.shape)
        w2 =w2.view(w2.shape[0], 1, -1)
        # print('shape of input w2', w2.shape)
        # print('shape of bias: ', bias.shape)
        # out = F.linear(
        #     w2, transform_matrix,
        #     bias = bias
        # ) 
        # print('shape of w2 mat mul transform matrix ',torch.matmul(w2, transform_matrix).shape )
        out = torch.matmul(w2, transform_matrix).view(w2.shape[0], -1)+ bias

        # print('successful computed output in Gaussian affine')
        # shape (current_out_style_dim, )
            # debug:
        # print('shape of input in EqualLinear:', input.shape)
        # print('shape of the weight in EqualLinear: ', self.weight.shape)
        return out

class GaussianModulatedConv2d(nn.Module):
    def __init__(
        self,
        in_channel, # number of input channels  
        out_channel, # number of output channels
        kernel_size, # size of each filter. In stylegan, this is set to 3 in style conv, and 1 in ToRGB layer
        prev_transformed_style_dim, #when applied, this should be equal to the number of input channels (in_channel) of the previous layers
        ...
        if use_external_affine:
            ...
            self.modulation = Gaussian_affine_map_with_prev_style(prev_transformed_style_dim = prev_transformed_style_dim, current_w_in_dim= w_style_dim, current_out_style_dim = in_channel, activation = activation_pre_affine)
    ...
    def forward(self,prev_transformed_style, input, current_style):
        ...
        if not self.fused:  # not the default
            ...
            if self.use_external_affine:
                ...
                style = self.modulation(prev_transformed_style,current_style) # output shape: (batch, in_channel)
        ...
        if self.use_external_affine:
            style = current_style.view(batch, 1, in_channel, 1, 1)
        else:
            style = self.modulation(prev_transformed_style, current_style).view(batch, 1, in_channel, 1, 1) # shape ( batch, 1, in_channel, 1, 1)
        ...
        return out, current_transformed_style
...
class StyledConv(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        print('initialized with activation: ', activation_pre_affine)
        self.conv = GaussianModulatedConv2d(
            in_channel,
            out_channel,
            kernel_size,
            prev_transformed_style_dim, # this is the prev_transformed_style dimension. Should be the same as the in_channel of the previous StyleConv Layer
            w_style_dim, # current w style dimension
            upsample=upsample,
            blur_kernel=blur_kernel,
            demodulate=demodulate,
            use_external_affine = use_external_affine,
            activation_pre_affine = activation_pre_affine
        )
    ...
    def forward(self, prev_transformed_style, input, current_w_style, noise=None):
        """
        takes three input:

        prev_transformed_style: should be a tensor that has shape (batch, prev_transformed_style_dim)
        input: 
        ...
        out, current_transformed_style = self.conv(prev_transformed_style, input, current_w_style)
...
class ToRGB(nn.Module):
    def __init__(self, in_channel, prev_transformed_style_dim , w_style_dim, upsample=True, blur_kernel=[1, 3, 3, 1], activation_pre_affine = None):
        super().__init__()

        if upsample:
            self.upsample = Upsample(blur_kernel)

        self.conv = GaussianModulatedConv2d(in_channel, 3, 1, prev_transformed_style_dim, w_style_dim,demodulate=False, activation_pre_affine = activation_pre_affine)# , activation_for_style= None)
        self.bias = nn.Parameter(torch.zeros(1, 3, 1, 1))
        print(f'ToRGB initialized, with in_channel: {in_channel}, prev_transformed_style_dim {prev_transformed_style_dim}, w_style_dim {w_style_dim}, ')

    def forward(self, prev_transformed_style, input, current_w_style, skip=None):
        ...
        out, _ = self.conv(prev_transformed_style,input, current_w_style)
        out = out + self.bias
...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        self.conv1 = StyledConv(
            self.channels[4], self.channels[4], 3, 
            1, # prev_transformed_style_dim
            ...
            activation_for_style = 'sqrt_softmax' # check! 
        ) 
        self.to_rgb1 = ToRGB(self.channels[4],
         self.channels[4], # prev_transformed_style_dim 
         w_style_dim, upsample=False, activation_pre_affine = activation_pre_affine )
        ...
        self.correct_for_first_w =  ThreeLayerFC(in_dim= w_style_dim, out_dim= self.channels[4], hidden_dim_1st=  n_neurons_middle_layer, hidden_dim_2nd = n_neurons_middle_layer, bias_init=0, activation = "fused_lrelu")
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(
                    ...
                    out_channel,
                    3, # kernel size
                    prev_in_channel, #prev_transformed_style_dim 
                   w_style_dim,
                    upsample=True,
                    blur_kernel=blur_kernel,
                    activation_pre_affine = activation_pre_affine
            ...
            self.convs.append(
                StyledConv(
                    ...
                    in_channel, # prev_transformed_style_dim 
                    w_style_dim, blur_kernel=blur_kernel,
                    activation_pre_affine = activation_pre_affine
                )
            )

            self.to_rgbs.append(ToRGB(out_channel, out_channel, w_style_dim, activation_pre_affine = activation_pre_affine))
    ...
    def get_inter_transformed_latent(self, latent): # originally, 'input' is used in place of 'latent'
        """
        changed:
        from the concatenated w, output a list of inter_transformed_latent.
        ...
        output:
        inter_transformed_latent, a list of length self.num_layers, each element of shape (batch, 32)
        only the first item was transformed
        ...
        inter_transformed_latent_list= []
        inter_transformed_latent_list.append(self.correct_for_first_w(latent[:,:self.w_style_dim]))
        for i in range(1,self.num_layers+1):
            inter_transformed_latent_list.append(latent[:, i*self.w_style_dim:(i*self.w_style_dim+self.w_style_dim)])
        
        assert len(inter_transformed_latent_list)==self.num_layers+1, 'not enough latent vectors to match number of layers'
        # print('checking shape of the original latent converted to a list:')
        # # for latent_32 in latent_list:
        # #     print(latent_32.shape)

        # # initialize a list of style vectors:
        # FC2_latent = []
        

        # # from the second latent onward, we use a 2-FC layer to transform it
        # for i in range(self.num_layers+1):
        #     converted_latent = self.from_w_to_correction_for_next[i](latent_list[i])
        ...
        #         correlated_latent.append(t_to_add)


        return inter_transformed_latent_list
    ...
    def forward(
        ...
        return_inter_transformed_latent=False,
        ...
        inter_transformed_latent = self.get_inter_transformed_latent(latent)
        #print('finished inter transformed \n')
        ...
        out = self.input(inter_transformed_latent[0])# result has nothing to do with the content of latent!

        zero_style = torch.zeros([latent.shape[0], 1]).to('cuda:0') # need to edit! 
        # print('\n computing first conv: \n')
        # print('dimensions of first input latent in first conv: ', inter_transformed_latent[0].shape)
        out, transformed_style_0 = self.conv1(zero_style, out, inter_transformed_latent[0], noise=noise[0])
        ...
        skip = self.to_rgb1(transformed_style_0, out, inter_transformed_latent[1])

        prev_transformed_style = transformed_style_0
        transformed_style_space_list = [inter_transformed_latent[0], prev_transformed_style]
        i = 1
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            self.convs[::2], self.convs[1::2], noise[1::2], noise[2::2], self.to_rgbs
        ):
            # print(f'\n computing {i+1}-th conv layer: \n')
            out, prev_transformed_style2 = conv1(prev_transformed_style, out, inter_transformed_latent[i], noise=noise1)
            # print(f'\n computing {i}-th conv layer: \n')
            out, prev_transformed_style3 = conv2(prev_transformed_style2,out, inter_transformed_latent[i + 1], noise=noise2)
            # print(f'\n computing {i}-th rgb layer \n ')
            skip = to_rgb(prev_transformed_style3,out, inter_transformed_latent[i + 2], skip)

            # add the transformed style into the style_space_list:
            transformed_style_space_list.append(prev_transformed_style2)
            transformed_style_space_list.append(prev_transformed_style3)

            # set the style for next iteration:
            prev_transformed_style = prev_transformed_style3
            i += 2
        ...
        if return_inter_transformed_latent:
            return image, inter_transformed_latent
