...
def make_noise(batch, latent_dim, n_noise, device , model_input_tensor = False, use_gauss_with_uniform_mix = False, uniform_mixing_ratio = 0.4,  use_accelerate = False):
    """
    ...
    if use_accelerate:
        if model_input_tensor == False:
            ...
            noises = torch.randn(batch, latent_dim)
...
def mixing_noise(batch, latent_dim, prob, device, model_input_tensor = False, use_accelerate = False):
    """
    ...
    if model_input_tensor==True:
        return make_noise(batch, latent_dim, 1, device, model_input_tensor=True, use_accelerate=use_accelerate)

    else:
        if prob > 0 and random.random() < prob:
            return make_noise(batch, latent_dim, 2, device, model_input_tensor=False, use_accelerate=use_accelerate
                              )

        else:
            return [make_noise(batch, latent_dim, 1, device, model_input_tensor = False, use_accelerate= use_accelerate)]
...
def train_mapping(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    if args.use_accelerate:
        ...
        print('accelerator is set.')
        device = accelerator.device
        print('using Accelerate. Device = ', device)
        r1_loss = torch.tensor(0.0)
        path_loss = torch.tensor(0.0)
        path_lengths = torch.tensor(0.0)

        generator, g_ema, discriminator, g_optim, d_optim, loader, torch.randn, torch.tensor, r1_loss, path_loss, path_lengths = \
            accelerator.prepare(generator, g_ema,  discriminator, g_optim, d_optim, loader, torch.randn, torch.tensor, r1_loss, path_loss, path_lengths)
    ...
    for idx in pbar:
        ...
        noise = mixing_noise(args.batch, args.latent, args.mixing, device = device, model_input_tensor=args.model_input_tensor, use_accelerate=args.use_accelerate)
        ...
        if args.use_accelerate:
            # print('backward with accelerator.')
        ...
        if d_regularize:
            ...
            if args.use_accelerate:
                accelerator.backward((args.r1 / 2 * r1_loss * args.d_reg_every + 0 * real_pred[0]))
            else:
        ...
        if args.use_g_regularization == True:
            ...
            if g_regularize:
                ...
                or args.arch=='multi_stage_GRU_Gauss_root_softmax' \
                    ...
                    or args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32_high_res' \
                ...
                if args.use_accelerate:
                    accelerator.backward(weighted_path_loss)
                else:
        ...
        if not os.path.exists(f"{'from_pretrained_high_res_'+args.arch+' '+args.activation_pre_affine}_sample"):
            os.makedirs(f"{'from_pretrained_high_res_'+args.arch+' '+args.activation_pre_affine}_sample")
        
        if not os.path.exists(f"{'from_pretrained_high_res_'+args.arch+' '+args.activation_pre_affine}_checkpoint"):
            os.makedirs(f"{'from_pretrained_high_res_'+args.arch+' '+args.activation_pre_affine}_checkpoint")
        ...
        if get_rank() == 0:
                ...
                # and save, from the same noise vector.
                    with torch.no_grad():
                        ...
                        if args.arch =='multi_stage_Gaussian_affine_with_activation' or \
                            ...
                            args.arch=="multi_stage_Gaussian_affine" or \
                            args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32_high_res' or \
                            args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32':
                        ...
                        utils.save_image(
                            sample,
                            f"{'from_pretrained_high_res_'+args.arch+' '+args.activation_pre_affine}_sample/{str(i).zfill(7)}.png",
            ...
            if i % 10000 == 0:
                torch.save(
                    {
                        "g_map": g_module.state_dict(),
                        "d": d_module.state_dict(),
                        "g_ema_map": g_ema.state_dict(),
                        "g_mapping_optim": g_optim.state_dict(),
                        "d_optim": d_optim.state_dict(),
                        "args": args,
                        "ada_aug_p": ada_aug_p,
                    },
                    f"{'from_pretrained_high_res_'+args.arch+' '+args.activation_pre_affine}_checkpoint/{str(i).zfill(7)}.pt",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--arch_of_original_model', type= str, default = 'stylegan2')
    parser.add_argument('--ckpt_of_original_model', type = str , default = None )
    ...
    parser.add_argument(
        "--use_accelerate",  action='store_false')
        #type = bool,
        # default = False,
        #action='store_false'
    ...
    # initial setting of device. Will be overiden inside the training function:
    ...
    if args.arch =='hypernetwork_with_2_layer_FC_offset_middle_dim_512_32' or args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32_high_res' or \
          args.arch =='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32':
    ...
    # load the original_model:

    if args.arch_of_original_model=='stylegan2':
        from model import Generator
        args.latent_original = 512
        args.n_mlp_original  = 8

    
    
    # load the checkpoint of the original model:
    ckpt_original = torch.load(args.ckpt_of_original_model, map_location= device )# lambda storage, loc: storage)

    original_g = Generator(
        args.size, args.latent_original, args.n_mlp_original, channel_multiplier=args.channel_multiplier
    ).to(device)

    original_g_ema = Generator(
    args.size,args.latent_original, args.n_mlp_original,  channel_multiplier=args.channel_multiplier
    ).to(device)
    ...
    print('importing model from', args.arch)

    if args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32_high_res':
        from Alvin_model_experiment_hypernetwork_offset_with_FC_on_3_FC_high_res import MyNewGenerator, Discriminator
    ...
    elif args.arch=='hypernetwork_with_2_layer_FC_offset_middle_dim_512_32' or \
        args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32_high_res' or \
            args.arch=='hypernetwork_offset_wth_FC_on_3_FC_middle_dim_512_32':
        g_ema = MyNewGenerator(args.size, w_style_dim = args.style_dim_for_changed_network,
        channel_multiplier=args.channel_multiplier,
    ...
    # only update the parameters of the 
    g_mapping_optim = optim.Adam(
        generator.FC_from_w_to_correction_for_next.parameters(),
        #generator.FC_from_w_to_correction_for_next.parameters(),
    ...
    d_optim = optim.Adam(
        ...
        # first load the main mapping network from the weights of main mapping network of original network:
        # to be added

    original_g.load_state_dict(ckpt_original["g"])
    original_g_ema.load_state_dict(ckpt_original["g_ema"])

    # extract the state_dict of just the main g network:
    # first, the input layer:
    generator.input.load_state_dict(original_g.input.state_dict())
    g_ema.input.load_state_dict(original_g_ema.input.state_dict())

    # conv1 layer:

    ## the ModulatedConv2d layer inside the StyledConv (conv1):
    generator.conv1.conv.weight = original_g.conv1.conv.weight
    g_ema.conv1.conv.weight = original_g_ema.conv1.conv.weight

    # # the NoiseInjection layer inside the CtyledConv
    generator.conv1.noise.weight = original_g.conv1.noise.weight
    g_ema.conv1.noise.weight= original_g_ema.conv1.noise.weight


    # to_rgb1:
    # 
    generator.to_rgb1.conv.weight = original_g.to_rgb1.conv.weight
    generator.to_rgb1.bias = original_g.to_rgb1.bias


    # for both convs and to_rgbs list:
    index_for_conv=0
    index_for_rgbs =0
    for i in range(3, generator.log_size+1):
        #g
        generator.convs[index_for_conv].conv.weight = original_g.convs[index_for_conv].conv.weight
        generator.convs[index_for_conv+1].conv.weight = original_g.convs[index_for_conv+1].conv.weight
        #ema
        g_ema.convs[index_for_conv].conv.weight = original_g_ema.convs[index_for_conv].conv.weight
        original_g_ema.convs[index_for_conv+1].conv.weight = original_g_ema.convs[index_for_conv+1].conv.weight

        #g:
        generator.to_rgbs[index_for_rgbs].conv.weight = original_g.to_rgbs[index_for_rgbs].conv.weight
        generator.to_rgbs[index_for_rgbs].bias= original_g.to_rgbs[index_for_rgbs].bias
        #ema:
        g_ema.to_rgbs[index_for_rgbs].conv.weight = original_g_ema.to_rgbs[index_for_rgbs].conv.weight
        g_ema.to_rgbs[index_for_rgbs].bias = original_g_ema.to_rgbs[index_for_rgbs].bias
        
        index_for_conv +=2
        index_for_rgbs +=1

    # deleting the unused models:
    del original_g, original_g_ema, ckpt_original
    ...
    # first constant layer:

    

    # del ckpt_original_g_input, ckpt_original_g_ema_input

    # # conv1:
    # generator.conv1.load_state_dict(ckpt_original_g_conv1)
    # g_ema.conv1.load_state_dict(ckpt_original_g_ema_conv1)

    # del ckpt_original_g_conv1, ckpt_original_g_ema_conv1

    # #to_rgb:
    # generator.to_rgb1.load_state_dict(ckpt_original_g_to_rgb1)
    # g_ema.to_rgb1.load_state_dict(ckpt_original_g_ema_to_rgb1)

    # del ckpt_original_g_to_rgb1, ckpt_original_g_ema_to_rgb1
    # #conv:
    # generator.conv.load_state_dict(ckpt_original_g_conv)
    # g_ema.conv.load_state_dict(ckpt_original_g_ema_conv)

    # del ckpt_original_g_conv, ckpt_original_g_ema_conv

    # # rgb:
    # generator.load_state_dict(ckpt_original_g_rgb)
    # g_ema.load_state_dict(ckpt_original_g_ema_rgb)

    # del ckpt_original_g_rgb, ckpt_original_g_ema_rgb

    if args.ckpt is not None:
        print("load model:", args.ckpt)

        ckpt = torch.load(args.ckpt, map_location= device) #lambda storage, loc: storage)
        ...
        # load the weights to be trained:
        generator.load_state_dict(ckpt["g_map"])
        generator = generator.to(device)
        g_ema.load_state_dict(ckpt["g_ema_map"])
        g_ema = g_ema.to(device)
        ...
        g_mapping_optim.load_state_dict(ckpt["g_mapping_optim"])
    ...
    if get_rank() == 0 and wandb is not None and args.wandb:
        wandb.init(project="high res PMSC-GAN")


    

 
    train_mapping(args, loader, generator, discriminator, g_mapping_optim, d_optim, g_ema, device)