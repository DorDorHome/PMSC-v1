# This model is changed from Alvin_model_experiment_changed_mapping_network_with_style_projection_to_sphere_scaled.py
...
#     The difference is that the output of StyleEqualLinear is a normalized output with spherical projection
...
#             # before going through normalization;, first multiply the style output by a constant
#             out = F.linear(input, self.weight * self.scale, bias=self.bias * self.lr_mul) # shape: (batch dim, out_dim )
#             out_norm_factor = torch.rsqrt(out.pow(2).sum([1]))  # shape :(batch,)
#             out = out*out_norm_factor.view(out.shape[0],1) # shape: (batch_dim, out_dim)
...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        style_dim, # dimension of each layer of the w space. This should be set to 32 in this new model
        ...
        self.to_rgb1 = ToRGB(self.channels[4], 2*style_dim, upsample=False)
        ...
        for i in range( self.num_layers):
            self.FC_from_w_to_correction_for_next.append(
                TwoLayerFC(in_dim= style_dim, out_dim= style_dim, middle_dim=  n_neurons_middle_layer, bias_init=0, activation = "fused_lrelu")
            )
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(
                    out_channel, out_channel, 3, 2*style_dim, blur_kernel=blur_kernel
                )
            )
    ...
    def get_correlated_latent(self, latent): # originally, 'input' is used in place of 'latent'
        ...
        assert latent.shape[1] == self.style_dim*(self.num_layers+1), "latent dimension disagrees with number of layers"       
        assert latent.requires_grad == True, 'latent do not require'
        # next, we turn latent into a list of vectors of shape(batch, 32, )
        latent_list= []
        for i in range(self.num_layers+1):
            latent_list.append(latent[:, i*self.style_dim:(i*self.style_dim+self.style_dim)])
        ...
        # first latent is unchanged:
        FC2_latent.append(latent_list[0]) 
        

        # from the second latent onward, we use a 2-FC layer to transform it
        for i in range(self.num_layers):
            converted_latent = self.FC_from_w_to_correction_for_next[i](latent_list[i+1])
        ...
        correlated_latent = [latent_list[0]]
        ...
        assert len(correlated_latent) == self.num_layers+1
    ...
    def forward(
        ...
        # into a list, of length self.num_layers, each element having shape (batch ,32)
        ...
        out = self.input(correlated_latent[0])# result has nothing to do with the content of latent!


        out = self.conv1(out, correlated_latent[0], noise=noise[0])
        skip = self.to_rgb1(out, correlated_latent[1])

        i = 1
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            self.convs[::2], self.convs[1::2], noise[1::2], noise[2::2], self.to_rgbs
        ):
            out = conv1(out, correlated_latent[i], noise=noise1)
            out = conv2(out, correlated_latent[i + 1], noise=noise2)
            skip = to_rgb(out, correlated_latent[i + 2], skip)
