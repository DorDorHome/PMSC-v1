...
class TwoLayerHypernetwork(nn.Module):
    ...
    """
    def __init__(self, in_dim_prev, in_dim_current, out_dim, middle_dim_main, middle_dim_hyper ,bias_init = 0, lr_mul_main = 1, lr_mul_offset = 1, scale_of_offset = 0.01, hidden_layer_activation = None, final_activation= None):
        super().__init__()
        self.in_dim_prev = in_dim_prev
        self.in_dim_current = in_dim_current

        self.middle_dim_main=middle_dim_main
        self.middle_dim_hyper = middle_dim_hyper
        ...
        self.FC_main_1 = EqualLinear(in_dim=in_dim_current, out_dim=middle_dim_main, bias = True, bias_init = bias_init, lr_mul = lr_mul_main, activation= hidden_layer_activation)
        self.weight_FC_main_2 = nn.Parameter(torch.randn(out_dim, middle_dim_main).div_(lr_mul_main))
        ...
        self.FC_offset_1 = EqualLinear(in_dim=in_dim_prev, out_dim=middle_dim_hyper, bias = True, bias_init = bias_init, lr_mul = lr_mul_offset, activation= hidden_layer_activation)
        self.FC_offset_2 = EqualLinear(in_dim=middle_dim_hyper, out_dim=out_dim*(middle_dim_main+1), bias = True, bias_init = bias_init, lr_mul = lr_mul_offset, activation= final_activation)
        
        # we also need to correct for the input variance
        self.scale_main_2 = 1/math.sqrt(middle_dim_main)*lr_mul_main
    ...
    def forward(self, prev_correlated_w, current_w):
        ...
        off_set_to_main_2 = self.FC_offset_1(prev_correlated_w)#shape (batch, middle_dim_hyper)
        off_set_to_main_2 = self.FC_offset_2(off_set_to_main_2)# shape (batch, out_dim*(middle_dim_main+1))
        off_set_to_main_2 = self.scale_of_offset*off_set_to_main_2.view(prev_correlated_w.shape[0], self.out_dim, self.middle_dim_main +1 ) # shape(batch, out_dim, middle_dim_main +1)
        ...
        # first layer of main
        main_out  = self.FC_main_1(current_w).view(current_w.shape[0], self.middle_dim_main, 1)# shape (batch, middle_dim_main)
        ...
        weight_FC_2 = self.weight_FC_main_2 + off_set_to_main_2[:, :, :self.middle_dim_main]# shape (batch, out_dim, middle_dim_main)
        ...
        if self.final_activation=="fused_lrelu":
            ...
            main_out = torch.matmul(weight_FC_2, main_out).view(current_w.shape[0], self.out_dim) + off_set_to_main_2[:,:,self.middle_dim_main]*self.lr_mul_main
...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        for i in range( self.num_layers):
            self.FC_from_w_to_correction_for_next.append(
                TwoLayerHypernetwork(in_dim_prev= w_style_dim, in_dim_current=w_style_dim, out_dim= w_style_dim,
                middle_dim_main= n_neurons_middle_layer,
                middle_dim_hyper= n_neurons_for_middle_layer_hypernet,
                lr_mul_main= 1,
                lr_mul_offset= 1,
