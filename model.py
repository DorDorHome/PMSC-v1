...
class Generator(nn.Module):
    def __init__(
        ...
        self.num_layers = (self.log_size - 2) * 2 + 1 # baseline is from size 4, log_size 2, meaning no upsize, 1 layer
    ...
    def forward(
        ...
        if len(styles) < 2:
            ...
            if styles[0].ndim < 3:#take the only element, and see if its ndim is 2 or less, meaning 
                # first unsqueeze, gives size( batch, 1, 512)
                # followed by repeat: 
                latent = styles[0].unsqueeze(1).repeat(1, inject_index, 1) # shape (batch, inject_index, 512)

            else:
                latent = styles[0] # shape (batch, inject_index, 512)
