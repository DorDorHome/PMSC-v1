...
class MyNewGenerator(nn.Module):
    def __init__(
        ...
        for i in range( self.num_layers):
            self.FC_from_w_to_correction_for_next.append(
                TwoLayerFC(in_dim= w_style_dim*2, out_dim= w_style_dim, middle_dim=  n_neurons_middle_layer, bias_init=0, hidden_layer_activation = mapping_network_hidden_layer_activation, final_activation = mapping_network_final_layer_activation)
            )
    ...
    def get_correlated_latent(self, latent): # originally, 'input' is used in place of 'latent'
        ...
        for i in range(1, self.num_layers+1):
            if i > 0:
                ...
                next_correlated_latent = self.FC_from_w_to_correction_for_next[i](input_for_next_FC)
